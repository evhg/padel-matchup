...
export async function snapshotMetrics(db: Db): Promise<void> {
  ...
  const [dbBytes, nPlayers, nEvents, nSlots, nPush] = await Promise.all([
    db
      .execute(sql`select pg_database_size(current_database()) as n`)
      .then((r) => Number((r as unknown as { rows?: { n: unknown }[] }).rows?.[0]?.n ?? (r as unknown as { n: unknown }[])[0]?.n ?? 0))
      .catch(() => 0),
    count(db.select({ n: sql<number>`count(*)` }).from(players)),
    count(db.select({ n: sql<number>`count(*)` }).from(events)),
    count(db.select({ n: sql<number>`count(*)` }).from(slots).where(inArray(slots.status, ["joined", "confirmed"]))),
    count(db.select({ n: sql<number>`count(*)` }).from(pushSubscriptions)),
  ]);
  await Promise.all([
    setMetric(db, "db_bytes", dbBytes),
    setMetric(db, "players_total", nPlayers),
    setMetric(db, "events_total", nEvents),
    setMetric(db, "slots_total", nSlots),
    setMetric(db, "push_subs", nPush),
  ]);
}
...
export async function activitySeries(db: Db, n: number, now = new Date()): Promise<DaySeries> {
  ...
  const [p, e, j] = await Promise.all([
    db.select({ d: sql<string>`to_char(${players.createdAt} at time zone 'UTC', 'YYYY-MM-DD')`, n: sql<number>`count(*)` }).from(players).where(gte(players.createdAt, since)).groupBy(sql`1`),
    db.select({ d: sql<string>`to_char(${events.createdAt} at time zone 'UTC', 'YYYY-MM-DD')`, n: sql<number>`count(*)` }).from(events).where(gte(events.createdAt, since)).groupBy(sql`1`),
    db
...
export async function totals(db: Db, now = new Date()): Promise<Totals> {
  ...
  const [playersN, eventsN, matches, tournaments, upcoming, joinedSlots, pushSubs] = await Promise.all([
    one(db.select({ n: sql<number>`count(*)` }).from(players)),
    one(db.select({ n: sql<number>`count(*)` }).from(events)),
    one(db.select({ n: sql<number>`count(*)` }).from(events).where(eq(events.type, "match"))),
    one(db.select({ n: sql<number>`count(*)` }).from(events).where(eq(events.type, "tournament"))),
    one(db.select({ n: sql<number>`count(*)` }).from(events).where(and(gte(events.startsAt, now), inArray(events.status, ["open", "full"])))),
    one(db.select({ n: sql<number>`count(*)` }).from(slots).where(inArray(slots.status, ["joined", "confirmed"]))),
    one(db.select({ n: sql<number>`count(*)` }).from(pushSubscriptions)),
  ]);
  const dbBytes = await db
    .execute(sql`select pg_database_size(current_database()) as n`)
    .then((r) => Number((r as unknown as { rows?: { n: unknown }[] }).rows?.[0]?.n ?? (r as unknown as { n: unknown }[])[0]?.n ?? 0))
    .catch(() => 0);
