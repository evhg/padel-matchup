...
/** The one place a personal link may send the device on to: an internal path (`/coach`, a match code, which may start with a digit), never a host or a query. */
export const safeNext = (next: string | null | undefined): string | null => (next && /^\/[a-z0-9][a-z0-9/_-]{0,40}$/i.test(next) ? next : null);
