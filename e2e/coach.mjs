...
try {
  ...
  // The next working day, not "tomorrow": Saturday and Sunday are off in this suite, so a run on a Friday or a Saturday would find no free times.
  const dayChips = olgaForm.locator('button[data-kind="day"]');
  let dayOffset = 1;
  for (; dayOffset < 4; dayOffset++) if (!/\b(?:Sat|Sun)\b/.test((await dayChips.nth(dayOffset).textContent()) ?? "")) break;
  const tomorrowChip = dayChips.nth(dayOffset);
