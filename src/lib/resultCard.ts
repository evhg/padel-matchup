...
type Versioned = Pick<EventDetail, "scores" | "event" | "roster">;
...
/** The result's version plus the photo's, when there is one. */
export const cardVersion = (detail: Versioned, photo: { createdAt: Date } | null) => `${resultVersion(detail)}${photo ? `-p${photo.createdAt.getTime().toString(36)}` : ""}`;
