...
- **When full:** per event, waitlist with auto-promotion on dropout (leave / removal / declined invite) or hard close. When a dropout leaves a seat **nobody was waiting for**, and the match is between 90 minutes and 48 hours away, one push goes to the people who would take it: the group's other members first, then the players with a match at that venue in the last 90 days or still to come, when the match is on the club's board. Once per match ever, only to players the level range admits and who turned push on, capped at 40, and never to the organizer or to whoever just left. A private match with no group tells nobody, by design.
