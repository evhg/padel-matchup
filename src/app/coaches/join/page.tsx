...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return { title, description, alternates: localeAlternates("/coaches/join", locale), openGraph: { title, description, type: "website", url: `${baseUrl()}/coaches/join` } };
}

type Props = { searchParams: Promise<{ s?: string | string[] }> };

/**
 * The coach's front door: what changes for them, in their words, and one button. It used to sit on
 * `/coaches`, which is the word a player types when they want a lesson — so a player looking for a
 * coach met a page selling an assistant to coaches, and left. `/coaches` is the player's list now;
 * this is where a coach lands. Indexed in three languages, and the door they came through (?s=) is
 * still counted when they set up.
 */
