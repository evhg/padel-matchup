...
try {
  ...
  const rpc = (body) => fetch(`${BASE}/mcp`, { method: "POST", headers: { "content-type": "application/json", accept: "application/json" }, body: JSON.stringify(body) }).then((r) => r.json());
  ...
  const tooEarly = await fetch(`${BASE}/api/v1/coaches/${handle}/lessons`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: annToken, startsAt: slotList[2] }) });
  ...
  const bookedRes = await fetch(`${BASE}/api/v1/coaches/${handle}/lessons`, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: annToken, startsAt: slotList[2] }) });
  ...
  const cancelRes = await fetch(`${BASE}/api/v1/coaches/${handle}/lessons/${bookedJ.lesson.id}`, { method: "DELETE", headers: { "content-type": "application/json" }, body: JSON.stringify({ token: annToken }) });
