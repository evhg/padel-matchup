...
export default async function CoachesPage() {
  ...
  const [busy, offers] = await Promise.all([busyForCoaches(db, ids, now, new Date(now.getTime() + 14 * DAY_MS)), offersForCoaches(db, ids)]);
  ...
  const card = (c: (typeof coaches)[number], cityName: string | null) => <CoachListCard key={c.id} coach={coachCardFacts(c, busy.get(c.id) ?? NO_BUSY, offers.get(c.id) ?? [], now)} locale={locale} foundingCity={cityName} />;
