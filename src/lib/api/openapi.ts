...
export function openapiDocument(base: string) {
  return {
    ...
    tags: [
      ...
      { name: "series", description: "Opens that repeat: a tournament series with its rhythm, the next edition to sign up for, the past podiums" },
    ...
    paths: {
      ...
      "/api/v1/series": {
        get: { tags: ["series"], operationId: "listSeries", summary: "Active series, optionally in one city, each with its next edition", parameters: [{ name: "city", in: "query", schema: { type: "string" }, description: "phuket or singapore" }], responses: { "200": jsonResponse("Series", { type: "object", properties: { city: { type: ["string", "null"] }, series: { type: "array", items: ref("Series") }, cities: { type: "array", items: { type: "object", properties: { slug: { type: "string" }, name: { type: "string" }, url: { type: "string" } } } }, note: { type: "string" } } }) } },
      },
      "/api/v1/series/{slug}": {
        get: { tags: ["series"], operationId: "getSeries", summary: "One series: rhythm, next edition, past editions with podiums", parameters: [{ name: "slug", in: "path", required: true, schema: { type: "string" } }], responses: { "200": jsonResponse("The series", ref("SeriesPage")), "404": jsonResponse("No such series", ref("Error")) } },
      },
      "/api/v1/clubs": {
    ...
    components: {
      ...
      schemas: {
        ...
        Series: { type: "object", properties: { slug: { type: "string" }, name: { type: "string" }, url: { type: "string" }, organizer: { type: ["string", "null"] }, format: { type: "string", enum: ["americano", "mexicano", "king"] }, rhythm: { type: "object", properties: { every: { type: "string", enum: ["week", "fortnight", "month"] }, weekday: { type: "integer", description: "0 = Sunday" }, time: { type: "string", description: "HH:MM in tz" }, nth: { type: ["integer", "null"], description: "Monthly: which weekday of the month, 5 = the last" }, tz: { type: "string" } } }, venue: { type: ["object", "null"], properties: { name: { type: "string" }, slug: { type: ["string", "null"] }, mapUrl: { type: ["string", "null"] } } }, level: { type: "object", properties: { min: { type: ["number", "null"] }, max: { type: ["number", "null"] }, verifiedOnly: { type: "boolean" } } }, capacity: { type: "integer" }, cost: { type: ["string", "null"] }, active: { type: "boolean" }, next: { type: ["object", "null"], properties: { code: { type: "string" }, url: { type: "string" }, startsAt: { type: "string", format: "date-time" }, spotsLeft: { type: "integer" } } } } },
        SeriesPage: { allOf: [ref("Series"), { type: "object", properties: { editions: { type: "integer" }, past: { type: "array", items: { type: "object", properties: { code: { type: "string" }, url: { type: "string" }, startsAt: { type: "string", format: "date-time" }, podium: { type: "array", items: { type: "object", properties: { name: { type: "string" }, rank: { type: "integer" } } } } } } } } }] },
        Club: { type: "object", properties: { slug: { type: "string" }, name: { type: "string" }, url: { type: "string" }, city: { type: ["string", "null"] }, country: { type: ["string", "null"], description: "ISO 3166-1 alpha-2" }, province: { type: ["string", "null"], description: "The place as the club named it" }, mapUrl: { type: ["string", "null"] }, website: { type: ["string", "null"] }, booking: { type: ["object", "null"], properties: { url: { type: "string" }, platform: { type: ["string", "null"], description: "playtomic, matchi, playbypoint, … or null for the club's own page" }, platformName: { type: ["string", "null"] } } }, courts: { type: ["integer", "null"] }, courtsIndoor: { type: ["integer", "null"], description: "Indoor courts of the total; null when the club has not said, 0 when none" }, courtsOutdoor: { type: ["integer", "null"] }, courtNames: { type: "array", items: { type: "string" }, description: "The courts by name, as the club listed them; empty until it does" }, about: { type: ["string", "null"] }, founding: { type: "boolean" }, freeCourts: { type: ["object", "null"], description: "Today's free court-hours from the club's own feed; null when the club shares none", properties: { day: { type: "string" }, tz: { type: "string" }, fetchedAt: { type: "string" }, slots: { type: "array", items: { type: "object", properties: { start: { type: "string" }, end: { type: "string" }, free: { type: "integer" } } } } } }, boardUrl: { type: "string" }, rankingUrl: { type: "string" }, calendarUrl: { type: "string" } } },
