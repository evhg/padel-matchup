...
export async function deliverWrap(p: Player, n: WrapNote): Promise<void> {
  if (emailEnabled() && p.email && p.emailNotifications) {
    const { html, text } = layout({ heading: n.heading, body: n.body, cta: { label: n.open, url: n.url }, footer: n.footer, eventUrl: n.url, openLabel: n.open, footerLink: { label: n.optOut, url: `${baseUrl()}${optOutPath(p.email)}` }, telegram: telegramLine(n.telegram, p) });
